...
using Content.Shared._Wega.Silicons.Borgs;
...
namespace Content.Server._Wega.Silicons.Borgs
{
    public sealed class AiRemoteControlSystem : SharedAiRemoteControlSystem
    {
        ...
        {
            AddComp<StationAiVisionComponent>(entity.Owner); // переменная visionComp не используется, можно убрать

            EntityUid? backAction = null;
            _actions.AddAction(entity.Owner, ref backAction, entity.Comp.BackToAiAction);
        ...
        private void OnShutdown(Entity<AiRemoteControllerComponent> entity, ref ComponentShutdown args)
        {
            ...
            _actions.RemoveAction(entity.Owner, entity.Comp.OpenRemoteDevicesMenuActionEntity);
        ...
        private void RewriteLaws(EntityUid from, EntityUid to)
        {
            ...
            if (!TryComp<SiliconLawProviderComponent>(to, out _))
                return;

            if (fromLawsComp.Lawset == null)
                return;

            _lawSystem.SetLawsSilent(fromLawsComp.Lawset.Laws, to);
        }
    }
}